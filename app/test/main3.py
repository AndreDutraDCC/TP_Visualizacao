...
fonemasALL = []
fonemasLABELS = ['pt', 'en', 'es', 'it', 'ru']
cores = ['green', 'blue', 'red', 'orange', 'purple', 'crimson']  # Adicione mais mapeamentos conforme necessário
...
pca = PCA(n_components=3)  # Defina o número de componentes desejado (2 para visualização em um gráfico)

# Aplicar o PCA aos seus dados
dados_reduzidos = pca.fit_transform(dados)

# Converter os dados reduzidos em um novo DataFrame
dados_reduzidos = pd.DataFrame(dados_reduzidos, columns=['Componente 1', 'Componente 2', 'Componente 3'])
...
ax.scatter(dados_reduzidos['Componente 1'], dados_reduzidos['Componente 2'], dados_reduzidos['Componente 3'], c=cores)
ax.set_xlabel('Componente 1')
ax.set_ylabel('Componente 2')
ax.set_zlabel('Componente 3')
ax.set_title('Redução de dimensionalidade usando PCA (3D)')
